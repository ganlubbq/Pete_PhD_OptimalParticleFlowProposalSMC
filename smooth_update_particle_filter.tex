...
We can design a system for adaptive step-size control using the concept of ``round-trip displacements''. Say the state has just been advanced deterministically from $\ls{\pt_0}$ to $\ls{\pt_1}$. We can then calculated the transformation required to move backwards in pseudo time to $\pt_0$ again. If the model were exactly linear Gaussian, then this would return us to $\ls{\pt_0}$ again. However, if we are using a Gaussian approximation then the move will not be precisely reversed, and we will reach a different location, $\bels{\pt_0}$. The larger the discrepancy between the Gaussian and the density it approximates, the larger will be the difference between $\ls{\pt_0}$ and $\bels{\pt_0}$, and also the further from optimal will be the flow. Thus, we can implement a system of step-size control by adjusting the interval lengths such that $\magdet{\ls{\pt_0} - \bels{\pt_0}}$ remains below a chosen threshold.
