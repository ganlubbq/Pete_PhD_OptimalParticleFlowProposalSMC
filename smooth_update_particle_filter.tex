...
\subsection{Nonlinear Gaussian Densities}
...
Such a solution will only work reliably when the log-densities are concave, and hence the Hessian matrices are negative definite at all points. This is the case for many exponential family distributions {\meta Erm... is it?}, but does not include heavy tailed distributions such as the student-t distribution.
...
%
\begin{IEEEeqnarray}{rCl}
 \epsilon_{S}(\pt_0,\pt_1) & = & \frac{1}{2} (\pt_1 - \pt_0) \times \epsilon_{D}(\pt_0,\pt_1) \nonumber      .
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
%
which is just a restatement of equation~\eqref{app-eq:lG_shift_basic}, from which $\lfshift{\pt}$ was originally derived. Thus, if step sizes are small, and $\lsdiff{\pt}$ is consequently small, then the flow is approximately optimal.
