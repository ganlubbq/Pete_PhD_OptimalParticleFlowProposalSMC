...
%
\begin{IEEEeqnarray}{rCl}
 \impden(\ls{\pt_1} | \ls{\pt_0}) & = & \normal{\ls{\pt_1}}{\lfmapmat \ls{\pt_0} + \lfmapshift}{\lfmapcov} \label{eq:lg_stochastic_flow_importance_density} \\
...
\begin{algorithmic}[1]
  \FOR{$t=1,2,\dots$}
    \FOR{$i=1,\dots,N_F$}
      ...
      \ENDIF
      \STATE Sample a predictive state value from the transition/prior density, $\ls{\rt,0} \sim p(\ls{\rt} | \ls{\rt-1})$ or $\ls{\rt,0} \sim p(\ls{\rt})$.
      \FOR{$l=1,2,...,L$, where $\pt{0}=0$, $\pt{L}=1$}
        ...
        \STATE Sample a state value from the smooth update importance density given by \eqref{eq:lg_stochastic_flow_importance_density}, $\ls{\rt,\pt{l}} \sim \impden(\ls{\rt,\pt{l}} | \ls{\rt,\pt{l-1}})$.
        \STATE Update particle weight, $\pw{\rt,\pt{l}}\pss{i}$ using \eqref{eq:deterministic_weight_update} or \eqref{eq:general_weight_update}.
      \ENDFOR
      \STATE Finalise, $\ls{\rt}\pss{i} = \ls{\rt,1}\pss{i}$, $\pw{\rt}\pss{i} = \pw{\rt,1}\pss{i}$.
    \ENDFOR
    \STATE Normalise weights, $\npw{\rt} = \pw{\rt}\pss{i} / \sum_j \pw{\rt}\pss{j}$ .
...
\end{IEEEeqnarray}
%
When $\lfdiffsf\rightarrow0$, $\lfmapmat\rightarrow0$, so this becomes a deterministic mapping. We also find very simply that,
%
\begin{IEEEeqnarray}{rCl}
 \lfmapmat \ls{\pt_0} + \lfmapshift & \rightarrow & \flowmap{\pt_0}{\pt_1}(\ls{\pt_0}) \nonumber      ,
\end{IEEEeqnarray}
%
and hence the state updates are identical. Examining the weight update and using $\ls{\pt_1} = \lfmapmat \ls{\pt_0} + \lfmapshift$, we find,
%
\begin{IEEEeqnarray}{rCl}
