...
\newcommand{\predpw}[1]{w_{#1}}                 % Predictive particle weight
...
In this case the particles are moved deterministically, and we can define a map between the states at $\pt_0$ and $\pt_1$,
%
\begin{IEEEeqnarray}{rCl}
 \flowmap{\pt_0}{\pt_1} (\ls{\rt,\pt_0}) & = & \ls{\rt,\pt_0} + \int_{\pt_0}^{\pt_1} \flowdrift{\pt}(\ls{\rt,l}) d\ls{\rt,l}     .
\end{IEEEeqnarray}
%
The weight update associated with the transition may now be evaluated using the standard formulas for the particle filter. Using the change of variables method, the distribution of the new particle states will be,
%
\begin{IEEEeqnarray}{rCl}
 \impden(\ls{1:\rt-1}, \ls{\pt_1}) & = & \augfiltden{\pt_0}(\ls{1:\rt-1}, \ls{\pt_0}) \magdet{ \frac{\partial \ls{\rt,\pt_0}}{\partial \ls{\rt,\pt_1}} }     .
\end{IEEEeqnarray}
%
Hence, assuming no resampling occurs,
%
\begin{IEEEeqnarray}{rCl}
...
When $\flowdiffuse{\pt}\ne0$, the naive approach of targeting $\augfiltden{\rt,\pt_1}$ directly does not work. If the proposal mechanism consists of selecting an existing particle from pseudo-time $\pt_0$ and then sampling from $\impden(\ls{\rt,\pt_1} | \ls{\rt,\pt_0}\pss{j})$, the resulting importance density is,
%
\begin{IEEEeqnarray}{rCl}
...
%
\begin{IEEEeqnarray}{rCl}
 \impden(\ls{\pt_1} | \ls{\pt_0}) & = & \normal{\ls{\pt_1}}{\lfmapmat{\pt_0,\pt_1} \ls{\pt_0} + \lfmapshift{\pt_0,\pt_1}}{\lfmapcov{\pt_0,\pt_1}} \label{eq:lg_stochastic_flow_importance_density} \\
 \lfmapmat{\pt_0,\pt_1}   & = & \exp\left\{ -\frac{1}{2} \lfdiffsf \left( \pt_1 - \pt_0 \right) \right\} \lginterm{\pt_1}^{-\frac{1}{2}} \lginterm{\pt_0}^{\frac{1}{2}} \nonumber \\
 \lfmapshift{\pt_0,\pt_1} & = & \lfmapmat{\pt_0,\pt_1} \left[ \lginterm{\pt_0}^{-1} \left( \obsmat^+ \ob{\rt} - \transmean \right) - \obsmat^+ \ob{\rt} \right] \nonumber \\
             &   & + \: \obsmat^+ \ob{\rt} - \lginterm{\pt_1}^{-1} \left( \obsmat^+ \ob{\rt} - \transmean \right) \nonumber \\
 \lfmapcov{\pt_0,\pt_1}   & = & \left[ 1 - \exp\left\{ - \lfdiffsf \left(\pt_1 - \pt_0\right) \right\} \right] \lgoicov{\pt_1}
\end{IEEEeqnarray}
%
where,
%
...
%
\begin{IEEEeqnarray}{rCl}
 \frac{ \artden(\ls{\pt_0} | \ls{\pt_1}) }{ \impden(\ls{\pt_1} | \ls{\pt_0}) } & = & \frac{ \oiden{\pt_0}(\ls{\pt_0} | \ls{\rt-1}) \impden(\ls{\pt_1} | \ls{\pt_0}) }{ \int \oiden{\pt_0}(\ls{} | \ls{\rt-1}) \impden(\ls{\pt_1} | \ls{}) d\ls{} \: \impden(\ls{\pt_1} | \ls{\pt_0}) } \nonumber \\
  & = & \frac{ \oiden{\pt_0}(\ls{\rt,\pt_0} | \ls{\rt-1}) }{ \int \oiden{\pt_0}(\ls{} | \ls{\rt-1}) \impden(\ls{\rt,\pt_1} | \ls{}) d\ls{} } \nonumber \\
  & = & \frac{ \normal{\ls{\pt_0}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} }{ \int \normal{\ls{}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} \normal{\ls{\pt_1}}{\lfmapmat{\pt_0,\pt_1} \ls{} + \lfmapshift{\pt_0,\pt_1}}{\lfmapcov{\pt_0,\pt_1}} d\ls{} } \nonumber \\
  & = & \frac{ \normal{\ls{\pt_0}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} }{ \normal{\ls{\pt_1}}{\lfmapmat{\pt_0,\pt_1} \lgoimean{\pt_0} + \lfmapshift{\pt_0,\pt_1}}{\lfmapmat{\pt_0,\pt_1}\lgoicov{\pt_0}\lfmapmat{\pt_0,\pt_1}^T + \lfmapcov{\pt_0,\pt_1}} } \nonumber \\
\end{IEEEeqnarray}
%
The weight update formula then simplifies to,
%
\begin{IEEEeqnarray}{rCl}
 \pw{\pt_1} & = & \pw{\pt_0} \frac{ \normal{\ob{\rt}}{\obsmat \ls{\pt_1}}{\frac{\obscov}{\pt_1}} \normal{\ls{\pt_1}}{\lgoimean{\pt_1}}{\lgoicov{\pt_1}} }{ \normal{\ls{\pt_1}}{\lfmapmat{\pt_0,\pt_1} \lgoimean{\pt_0} + \lfmapshift{\pt_0,\pt_1}}{\lfmapmat{\pt_0,\pt_1} \lgoicov{\pt_0} \lfmapmat{\pt_0,\pt_1}^T + \lfmapcov{\pt_0,\pt_1}} } \nonumber     .
\end{IEEEeqnarray}
...
In the stochastic case, the weight update formula will differ slightly to that used when the model was linear Gaussian. Since the OID is no longer a Gaussian, we cannot calculate the optimal artificial conditional density. Instead, we use the approximately optimal form,
%
\begin{IEEEeqnarray}{rCl}
 \frac{ \artden(\ls{\pt_0} | \ls{\pt_1}) }{ \impden(\ls{\pt_1} | \ls{\pt_0}) } & = & \frac{ \approxoiden{\pt_0}(\ls{\rt,\pt_0} | \ls{\rt-1}) }{ \int \approxoiden{\pt_0}(\ls{} | \ls{\rt-1}) \impden(\ls{\rt,\pt_1} | \ls{}) d\ls{} } \nonumber \\
  & = & \frac{ \normal{\ls{\pt_0}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} }{ \int \normal{\ls{}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} \normal{\ls{\pt_1}}{\lfmapmat{\pt_0,\pt_1} \ls{} + \lfmapshift{\pt_0,\pt_1}}{\lfmapcov{\pt_0,\pt_1}} d\ls{} } \nonumber \\
  & = & \frac{ \normal{\ls{\pt_0}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} }{ \normal{\ls{\pt_1}}{\lfmapmat{\pt_0,\pt_1} \lgoimean{\pt_0} + \lfmapshift{\pt_0,\pt_1}}{\lfmapmat{\pt_0,\pt_1}\lgoicov{\pt_0}\lfmapmat{\pt_0,\pt_1}^T + \lfmapcov{\pt_0,\pt_1}} } \nonumber       .
\end{IEEEeqnarray}
%
This is used in conjunction with~\eqref{eq:general_weight_update}. Note that from this formula it is easy to show that the stochastic flow weight update formula becomes equal to the deterministic flow weight update formula when $\lfdiffsf\rightarrow0$ (see appendix~\ref{app:limiting_case}).
...
We can design a system for adaptive step-size control using the concept of ``round-trip displacements''. Say the state has just been advanced deterministically from $\ls{\pt_0}$ to $\ls{\pt_1}$. We can then calculate the transformation required to move backwards in pseudo time to $\pt_0$ again. If the model were exactly linear Gaussian, then this would return us to $\ls{\pt_0}$. However, if we are using a Gaussian approximation then the move will not be precisely reversed, and we will reach a different location, $\bels{\pt_0}$. The larger the discrepancy between the Gaussian and the density it approximates, the larger will be the difference between $\ls{\pt_0}$ and $\bels{\pt_0}$, and also the further from optimal will be the flow. Thus, we can implement a system of step-size control by adjusting the interval lengths such that $\magdet{\ls{\pt_0} - \bels{\pt_0}}$ remains below a chosen threshold.

Note that,
%
\begin{IEEEeqnarray}{rCl}
 \ls{\pt_0} - \bels{\pt_0} & = & \ls{\pt_0} -  \left[ \lfmapmat{\pt_1,\pt_0} \ls{\pt_1} + \lfmapshift{\pt_1,\pt_0} \right] \nonumber \\
 & = & \ls{\pt_0} -  \left[ \lfmapmat{\pt_1,\pt_0} \left( \lfmapmat{\pt_0,\pt_1} \ls{\pt_0} + \lfmapshift{\pt_0,\pt_1} \right) + \lfmapshift{\pt_1,\pt_0} \right] \nonumber \\
 & = & \left[ I - \lfmapmat{\pt_1,\pt_0} \lfmapmat{\pt_0,\pt_1} \right] \ls{\pt_0} - \lfmapmat{\pt_1,\pt_0} \lfmapshift{\pt_0,\pt_1} - \lfmapshift{\pt_1,\pt_0}     .
\end{IEEEeqnarray}

{\meta This is not yet implemented, but monitoring the diagnostic suggests it should work. Mention how to cope with stochastic case --- use the mean rather than the sampled value.}
...
\begin{algorithm} \label{alg:nlg_SUPF}
\begin{algorithmic}[1]
  \FOR{$t=1,2,\dots$}
    \FOR{$i=1,\dots,N_F$}
      \IF{$t>1$ and $\ess{\rt-1}$ less than threshold}
      ...
      \ENDIF
      \STATE Sample a predictive state value from the transition/prior density, $\ls{\rt,0} \sim p(\ls{\rt} | \ls{\rt-1})$ or $\ls{\rt,0} \sim p(\ls{\rt})$.
      \FOR{$l=1,2,...,L$, where $\pt_{0}=0$, $\pt_{L}=1$}
        \STATE Calculate suitable local approximations for $\transmean$, $\transcov$, $\ob{\rt}$, $\obsmat$, $\obscov$ using linearisation, mixing variable sampling, or gradient matching.
        \STATE Sample a state value from the smooth update importance density given by \eqref{eq:lg_stochastic_flow_importance_density}, $\ls{\rt,\pt{l}} \sim \impden(\ls{\rt,\pt_{l}} | \ls{\rt,\pt_{l-1}})$.
        \STATE Update particle weight, $\pw{\rt,\pt_{l}}\pss{i}$ using \eqref{eq:deterministic_weight_update} or \eqref{eq:general_weight_update}.
      \ENDFOR
      \STATE Finalise, $\ls{\rt}\pss{i} = \ls{\rt,1}\pss{i}$, $\pw{\rt}\pss{i} = \pw{\rt,1}\pss{i}$.
    \ENDFOR
    \STATE Normalise weights, $\npw{\rt} = \pw{\rt}\pss{i} / \sum_j \pw{\rt}\pss{j}$ .
...
\begin{figure}
\centering
\subfloat[State]{\includegraphics[width=0.45\columnwidth]{example_particle_trajectories_nlg_rs0_kk7_deter_state_evo.pdf}}
\subfloat[Weight]{\includegraphics[width=0.45\columnwidth]{example_particle_trajectories_nlg_rs0_kk7_deter_weight_evo.pdf}}
...
\begin{figure}
\centering
\subfloat[State]{\includegraphics[width=0.45\columnwidth]{example_particle_trajectories_nlg_rs0_kk7_stoch_state_evo.pdf}}
\subfloat[State (detail)]{\includegraphics[width=0.45\columnwidth]{example_particle_trajectories_nlg_rs0_kk7_stoch_state_evo_zoom.pdf}}
...
\begin{IEEEeqnarray}{rCl}
 ...
 & = & \lginterm{\pt}^{-\frac{1}{2}} \lginterm{\pt_0}^{\frac{1}{2}} \ls{\pt_0} \nonumber \\
 &   & + \: \left[ \obsmat^+ \ob{\rt} - \lginterm{\pt}^{-1} \left( \obsmat^+ \ob{\rt} - \transmean \right) \right] \nonumber \\
 &   & - \: \lginterm{\pt}^{-\frac{1}{2}} \lginterm{\pt_0}^{\frac{1}{2}} \left[ \obsmat^+ \ob{\rt} - \lginterm{\pt_0}^{-1} \left( \obsmat^+ \ob{\rt} - \transmean \right) \right]     .
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
 ...
 \lfmapmat{\pt_0,\pt_1}   & = & \exp\left\{ -\frac{1}{2} \lfdiffsf \left( \pt_1 - \pt_0 \right) \right\} \lginterm{\pt_1}^{-\frac{1}{2}} \lginterm{\pt_0}^{\frac{1}{2}} \nonumber \\
 \lfmapshift{\pt_0,\pt_1} & = & \exp\left\{ -\frac{1}{2} \lfdiffsf \left( \pt_1 - \pt_0 \right) \right\} \lginterm{\pt_1}^{-\frac{1}{2}} \lginterm{\pt_0}^{\frac{1}{2}} \left[ \lginterm{\pt_0}^{-1} \left( \obsmat^+ \ob{\rt} - \transmean \right) - \obsmat^+ \ob{\rt} \right] \nonumber \\
             &   & + \: \obsmat^+ \ob{\rt} - \lginterm{\pt_1}^{-1} \left( \obsmat^+ \ob{\rt} - \transmean \right) \nonumber \\
 \lfmapcov{\pt_0,\pt_1}   & = & \left[ 1 - \exp\left\{ - \lfdiffsf \left(\pt_1 - \pt_0\right) \right\} \right] \lgoicov{\pt_1}
\end{IEEEeqnarray}
...
\section{The Limiting Case of the Stochastic Flow}\label{app:limiting_case}
...
%
\begin{IEEEeqnarray}{rCl}
 \frac{ \artden(\ls{\pt_0} | \ls{\pt_1}) }{ \impden(\ls{\pt_1} | \ls{\pt_0}) } & = & \frac{ \normal{\ls{\pt_0}}{\lgoimean{\pt_0}}{\lgoicov{\pt_0}} }{ \normal{\ls{\pt_1}}{\lfmapmat{} \lgoimean{\pt_0} + \lfmapshift{}}{\lfmapmat{}\lgoicov{\pt_0}\lfmapmat{}^T + \lfmapcov{}} } \nonumber \\
 & = & \frac{ \magdet{2 \pi \lgoicov{\pt_0}}^{-\frac{1}{2}} \exp\left\{ -\frac{1}{2} \left[ (\ls{\pt_1} - \lgoimean{\pt_0})^T \lgoicov{\pt_0}^{-1} (\ls{\pt_0} - \lgoimean{\pt_0}) \right] \right\} }{ \magdet{2 \pi \left( \lfmapmat{} \lgoicov{\pt_0} \lfmapmat{}^T + \lfmapcov{} \right)}^{-\frac{1}{2}} \exp\left\{ -\frac{1}{2} \left[ (\ls{\pt_0} - \lfmapmat{} \lgoimean{\pt_0} - \lfmapshift{})^T \left( \lfmapmat{} \lgoicov{\pt_0} \lfmapmat{}^T + \lfmapcov{} \right)^{-1} (\ls{\pt_1} - \lfmapmat{} \lgoimean{\pt_0} - \lfmapshift{}) \right] \right\} } \nonumber \\
 & \rightarrow & \frac{ \magdet{2 \pi \lgoicov{\pt_0}}^{-\frac{1}{2}} \exp\left\{ -\frac{1}{2} \left[ (\ls{\pt_0} - \lgoimean{\pt_0})^T \lgoicov{\pt_0}^{-1} (\ls{\pt_0} - \lgoimean{\pt_0}) \right] \right\} }{ \magdet{2 \pi \left( \lfmapmat{} \lgoicov{\pt_0} \lfmapmat{}^T \right)}^{-\frac{1}{2}} \exp\left\{ -\frac{1}{2} \left[ (\ls{\pt_1} - \lfmapmat{} \lgoimean{\pt_0} - \lfmapshift{})^T \left( \lfmapmat{} \lgoicov{\pt_0} \lfmapmat{}^T \right)^{-1} (\ls{\pt_1} - \lfmapmat{} \lgoimean{\pt_0} - \lfmapshift{}) \right] \right\} } \nonumber \\
 & = & \frac{ \exp\left\{ -\frac{1}{2} \left[ (\ls{\pt_0} - \lgoimean{\pt_0})^T \lgoicov{\pt_0}^{-1} (\ls{\pt_0} - \lgoimean{\pt_0}) \right] \right\} }{ \magdet{\lfmapmat{}^T \lfmapmat{}}^{-\frac{1}{2}} \exp\left\{ -\frac{1}{2} \left[ (\lfmapmat{}^{-1} \ls{\pt_1} - \lgoimean{\pt_0} - \lfmapmat{}^{-1} \lfmapshift{})^T \lgoicov{\pt_0}^{-1} (\lfmapmat{}^{-1} \ls{\pt_1} - \lgoimean{\pt_0} - \lfmapmat{}^{-1} \lfmapshift{}) \right] \right\} } \nonumber \\
 & = & \magdet{\lfmapmat{}^T \lfmapmat{}}^{\frac{1}{2}} \nonumber \\
 & = & \magdet{ \lginterm{\pt_1}^{-\frac{1}{2}} \lginterm{\pt_0}^{\frac{1}{2}} } \nonumber \\
 & = & \sqrt{ \frac{\magdet{\lginterm{\pt_0}}}{\magdet{\lginterm{\pt_1}}} }     ,
\end{IEEEeqnarray}
%
...
\section{Locally Optimal Flow for Nonlinear Gaussian Models}\label{app:linearised_optimal_flow}
...
The optimal flow cannot be calculated analytically when the observation function is nonlinear. However, if this function is approximated by the first two terms of the Taylor series, then the resulting linear model may be used in conjunction with the previously derived results for linear Gaussian models,
%
\begin{IEEEeqnarray}{rCl}
 \obsfun(\ls{}) & \approx & \obsfun(\ls{}^*) + \underbrace{\left.\frac{\partial \obsfun}{\partial \ls{}}\right|_{\ls{}^*}}_{\obsmat(\ls{}^*)} (\ls{} - \ls{}^*) \nonumber
\end{IEEEeqnarray}
\begin{IEEEeqnarray}{rCl}
 p(\ob{\rt} | \ls{\rt}) & \approx & \normal{\ob{\rt}}{\obsfun(\ls{}^*) + \obsmat(\ls{}^*)(\ls{\rt} - \ls{}^*)}{\obscov} \nonumber \\
\end{IEEEeqnarray}

It is now more natural to reformulate all the densities in terms of state and observation differences (omitting subscript $\rt$s).
%
\begin{IEEEeqnarray}{rCl}
 p(\ob{} | \ls{})                   & \approx & \normal{\obdiff{}}{\obsmat(\ls{}^*)\lsdiff{}}{\obscov} \nonumber \\
 p(\ls{} | \ls{\rt-1})              & = & \normal{\lsdiff{}}{\transmeandiff}{\transcov} \nonumber \\
 \oiden{\pt}(\ls{\pt} | \ls{\rt-1}) & = & \normal{\lsdiff{\pt}}{\lgoimeandiff{\pt}}{\lgoicov{\pt}} \nonumber \\
 \obdiff{}                          & = & \ob{} - \obsfun(\ls{}^*) \nonumber \\
 \lsdiff{}                          & = & \ls{} - \ls{}^* \nonumber \\
 \transmeandiff                     & = & \transmean - \ls{}^* \nonumber \\
 \lgoimeandiff{\pt}                 & = & \lgoicov{\pt} \left[ \transcov^{-1} \transmeandiff + \obsmat^T \left(\frac{\obscov}{\pt}\right)^{-1} \obdiff{} \right] \nonumber     .
\end{IEEEeqnarray}

Now, assuming a linear flow, the steps required are identical to those taken in the linear Gaussian case. The formulas for the drift, the optimal map and optimal importance density are all the same, but with $\ls{}$, $\ob{}$ and $\transmean$ replaced by $\lsdiff{}$, $\obdiff{}$ and $\transmeandiff$ respectively.

Next we consider the optimality of such a flow, starting with the deterministic case. First we rewrite equation~\eqref{eq:optimal_flow_PDE} which governs the optimal flow by subtracting from it the particular case when $\ls{\pt}=\ls{\pt}^*$,
%
\begin{IEEEeqnarray}{rCl}
  \IEEEeqnarraymulticol{3}{l}{ \log\left(\flowod(\ls{\pt})\right) - \log\left(\flowod(\ls{}^*)\right) } \nonumber \\
  \qquad & = & - \: \left[ \nabla\cdot \flowdrift{\pt}(\ls{\pt}) - \nabla\cdot \left.\flowdrift{\pt}(\ls{\pt})\right|_{\ls{}^*} \right] \nonumber \\
  \qquad &   & - \: \left[ \flowdrift{\pt}(\ls{\pt}) \cdot \nabla \log\left( \oiden{\pt}(\ls{\pt}) \right) - \flowdrift{\pt}(\ls{}^*) \cdot \nabla \left.\log\left( \oiden{\pt}(\ls{\pt}) \right)\right|_{\ls{}^*} \right] \nonumber \label{app-eq:central_optimal_flow_PDE}      ,
\end{IEEEeqnarray}
%
...
\begin{IEEEeqnarray}{rCl}
  \IEEEeqnarraymulticol{3}{l}{ \log\left(\flowod(\ls{\pt})\right) - \log\left(\flowod(\ls{}^*)\right) } \nonumber \\
  & = & -\frac{1}{2} (\ob{}-\obsfun(\ls{\pt}))^T \obscov^{-1} (\ob{}-\obsfun(\ls{\pt})) + \frac{1}{2} (\ob{}-\obsfun(\ls{}^*))^T \obscov^{-1} (\ob{}-\obsfun(\ls{}^*)) \nonumber \\
  & = & \ob{}^T \obscov^{-1} \left[ \obsfun(\ls{\pt}) - \obsfun(\ls{}^*) \right] - \frac{1}{2} \left[ \obsfun(\ls{\pt}) \obscov^{-1} \obsfun(\ls{\pt}) - \obsfun(\ls{}^*) \obscov^{-1} \obsfun(\ls{}^*) \right] \nonumber      .
\end{IEEEeqnarray}

Next, Taylor expand the functions $\obsfun(\ls{\pt})$ and $\flowdrift{\pt}(\ls{\pt})$,
%
\begin{IEEEeqnarray}{rCl}
 \obsfun(\ls{}) & = & \obsfun(\ls{}^*) + \tensorD{1}{\ls{}}{\obsfun} + \frac{1}{2} \tensorD{2}{\ls{}}{\obsfun} + \bigo{\lsdiff{}^3} \nonumber \\
 \flowdrift(\ls{}) & = & \flowdrift(\ls{}^*) + \tensorD{1}{\ls{}}{\flowdrift} + \frac{1}{2} \tensorD{2}{\ls{}}{\flowdrift} + \frac{1}{3!} \tensorD{3}{\ls{}}{\flowdrift} + \bigo{\lsdiff{}^4} \nonumber     ,
\end{IEEEeqnarray}
%
where,
%
\begin{IEEEeqnarray}{rCl}
 \tensorD{n}{\ls{}}{f} & = & \sum_{i_1} \lsdiff{i_1} \dots \sum_{i_n} \lsdiff{i_n} \left.\frac{\partial^n f}{\partial \ls{i_1} \dots \partial \ls{i_n} }\right|_{\ls{}^*} \nonumber      ,
\end{IEEEeqnarray}
%
and here, $\ls{i_n}$ and $\lsdiff{i_n}$ are the ($i_n$)th components of $\ls{}$ and $\lsdiff{}$ respectively. Obviously we also have,
%
\begin{IEEEeqnarray}{rCl}
 \tensorD{1}{\ls{}}{\obsfun} & = & \obsmat(\ls{}^*) \lsdiff{} \nonumber     .
\end{IEEEeqnarray}

Using the expansion, we find,
%
\begin{IEEEeqnarray}{rCl}
  \IEEEeqnarraymulticol{3}{l}{ \log\left(\flowod(\ls{\pt})\right) - \log\left(\flowod(\ls{}^*)\right) } \nonumber \\
  \qquad & = & \ob{}^T \obscov^{-1} \left[ \obsfun(\ls{\pt}) - \obsfun(\ls{}^*) \right] - \frac{1}{2} \left[ \obsfun(\ls{\pt}) \obscov^{-1} \obsfun(\ls{\pt}) - \obsfun(\ls{}^*) \obscov^{-1} \obsfun(\ls{}^*) \right] \nonumber \\
  \qquad & = & \ob{}^T \obscov^{-1} \left[ \obsmat(\ls{}^*) \lsdiff{\pt} + \frac{1}{2} \tensorD{2}{\ls{}}{\obsfun} \right] \nonumber \\
  \qquad &   & \qquad - \: \frac{1}{2} \left[ 2 \obsfun(\ls{}^*)^T \obscov^{-1} \obsmat(\ls{}^*) \lsdiff{\pt} + \lsdiff{\pt}^T \obsmat(\ls{}^*)^T \obscov^{-1} \obsmat(\ls{}^*) \lsdiff{\pt} + \obsfun(\ls{}^*)^T \obscov^{-1} \tensorD{2}{\ls{}}{\obsfun} \right] \nonumber \\
  \qquad &   & \qquad + \: \bigo{\lsdiff{\pt}^3} \nonumber     ,
\end{IEEEeqnarray}
%
\begin{IEEEeqnarray}{rCl}
 \IEEEeqnarraymulticol{3}{l}{ \nabla \log\left( \oiden{\pt}(\ls{\pt}) \right) } \nonumber \\
 \qquad & = & - \transcov^{-1} (\ls{\pt}-\transmean) + \obsmat(\ls{}^*)^T \obscov^{-1}(\ob{}-\obsfun(\ls{\pt})) \nonumber \\
 & = & - \transcov^{-1} (\lsdiff{\pt}-\transmeandiff) + \obsmat(\ls{}^*)^T \obscov^{-1}\left(\ob{}- \left(\obsfun(\ls{}^*) + \obsmat(\ls{}^*) \lsdiff{\pt} + \frac{1}{2} \tensorD{2}{\ls{}}{\obsfun} + \bigo{\lsdiff{\pt}^3}\right) \right) \nonumber \\
 & = & - \left[\transcov^{-1} + \obsmat(\ls{}^*)^T \obscov^{-1} \obsmat(\ls{}^*)\right] \lsdiff{\pt} + \left[\transcov^{-1} \transmeandiff + \obsmat(\ls{}^*)^T \obscov^{-1} \obdiff{}\right] - \frac{1}{2} \obsmat(\ls{}^*)^T \obscov^{-1} \tensorD{2}{\ls{}}{\obsfun} + \bigo{\lsdiff{\pt}^3} \nonumber \\
 & = & - \lgoicov{\pt}^{-1} \left[ \lsdiff{\pt} - \lgoimeandiff{\pt} \right] - \frac{1}{2} \obsmat(\ls{}^*)^T \obscov^{-1} \tensorD{2}{\ls{}}{\obsfun} + \bigo{\lsdiff{\pt}^3} \nonumber      ,
\end{IEEEeqnarray}
%
\begin{IEEEeqnarray}{rCl}
 \IEEEeqnarraymulticol{3}{l}{ \left[ \flowdrift{\pt}(\ls{\pt}) \cdot \nabla \log\left( \oiden{\pt}(\ls{\pt}) \right) - \flowdrift{\pt}(\ls{}^*) \cdot \nabla \left.\log\left( \oiden{\pt}(\ls{\pt}) \right)\right|_{\ls{}^*} \right] } \nonumber \\
 \qquad & = & - \flowdrift{\pt}(\ls{}^*)^T \lgoicov{\pt}^{-1} \lsdiff{\pt} - \frac{1}{2} \flowdrift{\pt}(\ls{}^*)^T \obsmat(\ls{}^*)^T \obscov^{-1} \tensorD{2}{\ls{}}{\obsfun} - \left(\tensorD{1}{\ls{}}{\flowdrift}\right)^T \lgoicov{\pt}^{-1} \left(\lsdiff{\pt}-\lgoimeandiff{\pt}\right) \nonumber \\
 \qquad &   & \qquad + \: \frac{1}{2} \tensorD{2}{\ls{}}{\flowdrift} \lgoicov{\pt}^{-1} \lgoimeandiff{\pt} + \bigo{\lsdiff{\pt}^3} \nonumber     ,
\end{IEEEeqnarray}
%
\begin{IEEEeqnarray}{rCl}
 \nabla\cdot \flowdrift{\pt}(\ls{\pt}) - \nabla\cdot \left.\flowdrift{\pt}(\ls{\pt})\right|_{\ls{}^*} & = & \divtensorD{2}{\ls{}}{\flowdrift{\pt}} + \frac{1}{2} \divtensorD{3}{\ls{}}{\flowdrift{\pt}} + \bigo{\lsdiff{\pt}^3} \nonumber      ,
\end{IEEEeqnarray}
%
where the divergence of the tensor derivative operator is,
%
\begin{IEEEeqnarray}{rCl}
 \divtensorD{n}{\ls{}}{f} & = & \sum_{i_1} \lsdiff{i_1} \dots \sum_{i_{n-1}} \lsdiff{i_{n-1}} \sum_k \left.\frac{\partial^n f_k}{\partial \ls{i_1} \dots \partial \ls{i_{n-1}} \partial \ls{k} }\right|_{\ls{}^*} \nonumber      .
\end{IEEEeqnarray}

Equation~\eqref{app-eq:central_optimal_flow_PDE} which governs the optimal flow has no constant terms. Hence, as $\lsdiff{\pt}\rightarrow0$, the linear terms will dominate, leaving,
%
\begin{IEEEeqnarray}{rCl}
 \obdiff{}^T \obscov^{-1} \obsmat(\ls{}^*) \lsdiff{\pt} & = & - \divtensorD{2}{\ls{}}{\flowdrift{\pt}} + \flowdrift{\pt}(\ls{}^*)^T \lgoicov{\pt}^{-1} \lsdiff{\pt} - \lgoimeandiff{\pt}^T \lgoicov{\pt}^{-1} \tensorD{1}{\ls{}}{\flowdrift} \nonumber      .
\end{IEEEeqnarray}

For our chosen linear flow,
%
\begin{IEEEeqnarray}{rCl}
 \flowdrift{\pt}(\ls{\pt}) & = & \lfmat{\pt} \lsdiff{\pt} + \lfshift{\pt} \nonumber \\
 \flowdrift{\pt}(\ls{}^*) & = & \lfshift{\pt} \nonumber \\
 \tensorD{1}{\ls{}}{\flowdrift} & = & \lfmat{\pt} \lsdiff{\pt} \nonumber \\
 \divtensorD{2}{\ls{}}{\flowdrift{\pt}} & = & 0 \nonumber     ,
\end{IEEEeqnarray}
%
so for the governing equation to be locally satisfied, we simply require,
%
\begin{IEEEeqnarray}{rCl}
 \obsmat(\ls{}^*)^T \obscov^{-1} \obdiff{} & = & \lgoicov{\pt}^{-1} \lfshift{\pt} - \lfmat{\pt}^T \lgoicov{\pt}^{-1} \lgoimeandiff{\pt} \nonumber      ,
\end{IEEEeqnarray}
%
which is simply a restatement of equation~\eqref{app-eq:lG_shift_basic}, from which $\lfshift{\pt}$ was originally derived. Thus, if step sizes are small, and $\lsdiff{\pt}$ is consequently small, then the flow is approximately optimal.
