...
When $\flowdiffuse{\pt}=0$, the optimal choice for $\flowdrift{\pt}$ in the deterministic case is given by,
%
\begin{IEEEeqnarray}{rCl}
...
\begin{IEEEeqnarray}{rCl}
 ...
 \lfshift{\pt} & = & \left[ I + 2 \pt \lfmat{\pt} \right] \left[ (I + \pt \lfmat{\pt}) \transcov \obsmat^T \obscov^{-1} \ob{\rt} + \lfmat \transfun(\ls{\rt-1}) \right]     .
\end{IEEEeqnarray}
%
For proof, see Appendix~\ref{app:optimal_flow_linear_Gaussian}.

For this optimal choice of $\flowdrift{\pt}$, the map associated with a transition form $\pt_0$ to $\pt_1$ is,
%
\begin{IEEEeqnarray}{rCl}
 \flowmap{\pt_0}{\pt_1} (\ls{\rt,\pt_0}) & = &
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 \magdet{ \frac{\partial \ls{\rt,\pt_1}}{\partial \ls{\rt,\pt_0}} } & = &
\end{IEEEeqnarray}
...
%
\begin{IEEEeqnarray}{rCl}
 \log\left( \oiden{\pt}(\ls{\pt}) \right) & = & \log\left( \alpha(\ls{\pt}) \right) + \pt \log\left( \beta(\ls{\pt}) \right) - \log\left(\lam{K}\right)     ,
\end{IEEEeqnarray}
%
\begin{IEEEeqnarray}{rCl}
 \frac{\partial}{\partial \pt} \log\left( \oiden{\pt}(\ls{\pt}) \right) & = & \frac{ 1 }{ \oiden{\pt}(\ls{\pt}) } \frac{\partial \oiden{\pt}}{\partial \pt} \nonumber \\
  & = & \log\left(\beta(\ls{\pt})\right) - \frac{d}{d\pt}\log\left(\lam{K}\right) \label{eq:dpi-dlam}     ,
\end{IEEEeqnarray}
%
\begin{IEEEeqnarray}{rCl}
...
\begin{IEEEeqnarray}{rCl}
 \frac{\partial \oiden{\pt}}{\partial \pt} & = & -\nabla \cdot \left[ \flowdrift{\pt}(\ls{\pt}) \oiden{\pt}(\ls{\pt}) \right] + \nabla \cdot \left[ \flowcov{\pt} \nabla \oiden{\pt}(\ls{\pt}) \right]     \nonumber \\
 \oiden{\pt}(\ls{\pt}) \left[ \log\left(\beta(\ls{\pt})\right) - \frac{d}{d\pt}\log\left(\lam{K}\right) \right] & = & -\nabla\cdot \flowdrift{\pt}(\ls{\pt}) \oiden{\pt}(\ls{\pt}) - \flowdrift{\pt}(\ls{\pt}) \cdot \nabla \oiden{\pt}(\ls{\pt}) + \nabla \cdot \left[ \flowcov{\pt} \nabla \oiden{\pt}(\ls{\pt}) \right] \nonumber \\
 \left[ \log\left(\beta(\ls{\pt})\right) - \frac{d}{d\pt}\log\left(\lam{K}\right) \right] & = & -\nabla\cdot \flowdrift{\pt}(\ls{\pt}) - \flowdrift{\pt}(\ls{\pt}) \cdot \nabla \log\left( \oiden{\pt}(\ls{\pt}) \right) + \frac{ 1 }{ \oiden{\pt}(\ls{\pt}) } \nabla \cdot \left[ \flowcov{\pt} \nabla \oiden{\pt}(\ls{\pt}) \right]      .
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 \frac{d}{d\pt}\log\left(\lam{K}\right) & = & \frac{\frac{dK}{d\pt}}{\lam{K}} \nonumber \\
                                               & = & \frac{ \int \alpha(\ls{\pt}) \beta(\ls{\pt})^\pt \log\left(\beta(\ls{\pt})\right) d\ls{\rt} }{ \int \alpha(\ls{\pt}) \beta(\ls{\pt})^\pt d\ls{\pt} } \nonumber \\
                                               & = & \mathbb{E}_{\oiden{\pt}}\left[ \log\left(\beta(\ls{\pt})\right) \right]     .
\end{IEEEeqnarray}
%
Thus,
%
