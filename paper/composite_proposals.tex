...
\end{IEEEeqnarray}
%
where $\lgoimeanapprox{\pt}{\lsfixed}$ and $\lgoicovapprox{\pt}{\lsfixed}$ are the mean and covariance formed at the point $\lsfixed$, which are themselves functions of $\pt$. Methods for making such an approximation are discussed in section~\ref{}, and employ linearisation or matching techniques at the specified point.
...
{\meta Devolve all the discussion to the appendix, just leaving the final choice. Find that differential equation paper to see if there are any other options. I think there will be, but they will have $\eta$ depending on $x$ and will thus not be integrable.}

We first require $\stdnorm{\pt}$ to be continuous, a sensible precaution since jumps in $\stdnorm{\pt}$ will cause jumps in $\ls{\pt}$ away from the approximation point. Then in order to maintain the correct marginal distribution, $\stdnorm{\pt}$ must evolve according to a stationary Ornstein-Uhlenbeck process governed by,
%
\begin{IEEEeqnarray}{rCl}
 d\stdnorm{\pt} & = & \oudrift{\pt} \stdnorm{\pt} d\pt + \oudiffuse{\pt} d\lginfbm{\pt} \nonumber      ,
\end{IEEEeqnarray}
%
subject to $\oudiffuse{\pt}\oudiffuse{\pt}^T = \oudrift{\pt}+\oudrift{\pt}^T$. Furthermore, in order to avoid introducing arbitrary biases in particular directions, $\oudiffuse{\pt}$ should be a scaled identity matrix and $\oudrift{\pt}$ should be positive semidefinite (see Appendix~\ref{app:stationary_ou_process}). Finally, for analytical tractability we require $\oudrift{\pt}$ and $\oudiffuse{\pt}$ to be constant over time. Together, these conditions leave us with the following stochastic differential equation (SDE),
%
\begin{IEEEeqnarray}{rCl}
 d\stdnorm{\pt} & = & -\half \lgexpsf \stdnorm{\pt} d\pt + \lgexpsf^{\half} d\lginfbm{\pt} \label{eq:standard_normal_SDE}     ,
\end{IEEEeqnarray}
%
where $\lgexpsf$ is a non-negative scale factor which determines the rate at which $\stdnorm{\pt}$ ``forgets'' its previous states. When $\lgexpsf=0$, $\stdnorm{\pt}$ is constant. This special case was considered in \citep{}{\meta Cite conference paper}.



\subsubsection{Finite Updates}
...
Hence, the state may be updated from $\pt_0$ to $\pt_1$ using \eqref{eq:gaussian_decomposition} along with a Gaussian approximation of the OID sequence formed at $\ls{\pt_0}$,
%
\begin{IEEEeqnarray}{rCl}
 \ls{\pt_1} & = & \lgoimeanapprox{\pt_1}{\lsfixed} + \lgupdmeanmat{\pt_0,\pt_1}(\ls{\pt_0}-\lgoimeanapprox{\pt_0}{\lsfixed}) + \lgupdcov{\pt_0,\pt_1}^{\half} \stdnorm{\Delta} \nonumber \\
 \lgupdmeanmat{\pt_0,\pt_1} & = & \exp\left\{-\half\lgexpsf(\pt_1-\pt_0)\right\} \lgoicovapprox{\pt_1}{\lsfixed}^{\half}\lgoicovapprox{\pt_0}{\lsfixed}^{-\half} \nonumber \\
 \lgupdcov{\pt_0,\pt_1} & = & \left[1-\exp\left\{-\lgexpsf(\pt_1-\pt_0)\right\}\right]\lgoicovapprox{\pt_1}{\lsfixed} \label{eq:state_update}      .
\end{IEEEeqnarray}
{\meta Change the $\lsfixed$s.}
...
In general, particles progress stochastically over pseudo-time. However, when $\lgexpsf=0$, the state evolution is entirely deterministic. {\meta Discuss this choice.}



\subsubsection{Continuous State Evolution}

Differentiating \eqref{eq:gaussian_decomposition} and combining with \eqref{eq:standard_normal_SDE}, we also obtain the following SDE for the state evolution, which will prove useful later when considering step-size control and optimality,
%
\begin{IEEEeqnarray}{rCl}
 d\ls{\pt} & = & \left[ \frac{\partial \lgoimeanapprox{\pt}{\lsfixed}}{\partial \pt} + \half \left( \frac{\partial \lgoicovapprox{\pt}{\lsfixed} }{\partial \pt} \lgoicovapprox{\pt}{\lsfixed}^{-1} - \lgexpsf I \right) (\ls{\pt}-\lgoimeanapprox{\pt}{\lsfixed}) \right] d\pt + \lgexpsf^{\half} \lgoicovapprox{\pt}{\lsfixed}^{\half} d\lginfbm{\pt} \label{eq:state_SDE}      .
\end{IEEEeqnarray}
%
...
The Fokker-Planck equation relates the motion of a particle with the evolution of its state distribution. For a particle to be distributed according to $\oiden{\pt}$ for all $\pt$, it must evolve according to the following SDE,
%
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
{\meta Change this to be completely in terms of log-density.}
%
where
%
\begin{IEEEeqnarray}{rCl}
...
This condition for the optimal state evolution in differential form sheds some light on the effect of step sizes and the Gaussian approximation used. In order to have the state evolve exactly according to the OID sequence, three quantities are needed, the gradient and Hessian of $\log\left(\oiden{\pt}(\ls{\pt})\right)$ at $\lsfixed$, and the expectation $\expect{\oiden{\pt}}\left[ \log\left(\flowod(\ls{\pt})\right) \right]$. The first two may be calculated analytically (provided both the transition and observation density are twice differentiable), while the latter is generally intractable.

Suppose that a Gaussian approximation could be made which exactly matched the gradient and Hessian of the log-density at a single point. If this single point were the current state, then as the update step size becomes small, the errors arising from these these terms will tend to zero. However, whatever the choice of approximation the error arising from the expectation term will remain, meaning that not even in the infinitesimal limit of the step sizes will a sample exactly from the OID be produced. 

In the case where $\lgexpsf=0$, in which the state evolution becomes deterministic, we of course have $\flowcov{\pt}=0$, and so it is only necessary for the approximation to match the gradient of the log-density and not the Hessian. This is often achievable. Because the covariance of a Gaussian must always be positive definite, matching both the gradient and Hessian can rarely be accomplished (and even when it can, it will sometimes lead to very poor approximations of the expectation term), so in general when $\lgexpsf>0$ the algorithm is unlikely to work as well, even with infinitesimal step sizes. Balanced against this, using a larger value of $\lgexpsf$ allows multiple copies the same particle to better explore the state space, reducing inter-particle dependence. This is a useful property when introducing resample-move schemes, as discussed in section~\ref{}({\meta Add ref}).

If an approximation is used which does match the gradient and Hessian at the current state, or at least is closest at this point, then the choice of dynamics should aim to minimise the distance which the particle travels away from this point during the succeeding update. This justifies our decision to require $\stdnorm{\pt}$ to evolve smoothly without jumps. Furthermore, it also suggests that small values of $\lgexpsf$ are preferable, since the stochastic component of the state update will dominate the particle motion for small steps sizes.
...
\end{IEEEeqnarray}
%
In general this is intractable so approximations are required. Using the existing Gaussian approximation \eqref{eq:gaussian_oid_approximation} for $\oiden{\pt}$,
%
\begin{IEEEeqnarray}{rCl}
 \artden(\ls{\pt_0} | \ls{\pt_1}) & = & \frac{ \approxoiden{\pt_0}{\lsfixed}(\ls{1:\rt-1}, \ls{\pt_0}) \impden(\ls{\pt_1} | \ls{\pt_0}) }{ \int \approxoiden{\pt_0}{\lsfixed}(\ls{1:\rt-1}, \ls{\pt_0}) \impden(\ls{\pt_1} | \ls{\pt_0}) d\ls{\pt_0} } \nonumber \\
...
\end{IEEEeqnarray}
{\meta Missing negative signs - check!}
%
where
%
\begin{IEEEeqnarray}{rCl}
...
\section{Ornstein-Uhlenbeck Processes With A Standard Gaussian Stationary Distribution} \label{app:stationary_ou_process}
