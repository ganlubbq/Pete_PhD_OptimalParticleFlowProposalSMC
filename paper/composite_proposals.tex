...
The purpose of the CPPF is to provide more efficient proposals for problems {\meta Something about its raison d'etre --- to improve ESS for difficult models.}
...
\begin{IEEEeqnarray}{rCl}
 ...
 \lgoimeanapprox{\pt}{\lsfixed} & = & \lsfixed + \lgoicov{\pt} \left[ \transcov^{-1} (\transfun(\ls{\rt-1})-\lsfixed) + \pt \obsmatapprox{\lsfixed}^T \obscov^{-1} (\ob{\rt}-\obsfun(\lsfixed)) \right] \label{eg:linearised_Gaussian_approx}     .
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
 ...
 \lgoimeanapprox{\pt}{\lsfixed} & = & \lsfixed + \lgoicovapprox{\pt}{\lsfixed} \left[ \frac{\partial}{\partial \ls{\pt}} \log\left(\oiden{\rt,\pt}\right) \right] \label{eg:general_Gaussian_approx}      .
\end{IEEEeqnarray}
%
This approximation may only be used when the Hessian of the log-density is negative definite, otherwise the matrix square root may not exist and the state update will not work. Intuitively, a Gaussian, whose log-density always has a negative curvature, cannot be used to approximate a log-density which curves upwards. If this condition is not satisfied then modifications may be used, for example by subtracting a scaled identity matrix from the Hessian to force the eigenvalues to be negative, or by diagonalising the Hessian and setting the offending negative eigenvalues to a negative constant.

{\meta REWRITE/AUGMENT THIS BIT.}
...
\end{IEEEeqnarray}
%
where $T$ is the sampling period of the sensor. Each observation consists of $50$ time samples and the observation density is modelled as a Gaussian with a covariance matrix $0.2^2 I$.
