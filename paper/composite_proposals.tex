...
%
\begin{IEEEeqnarray}{rCl}
 d\ls{\pt} & = & \left[ \pdv{\lgoimeanapprox{\pt}{\pt_0}}{\pt} + \half \left( \pdv{\lgoicovapprox{\pt}{\pt_0}}{\pt} \lgoicovapprox{\pt}{\pt_0}^{-1} - \lgexpsf I \right) (\ls{\pt}-\lgoimeanapprox{\pt}{\pt_0}) \right] d\pt + \lgexpsf^{\half} \lgoicovapprox{\pt}{\pt_0}^{\half} d\lginfbm{\pt} \label{eq:state_SDE}      .
\end{IEEEeqnarray}
%
...
\begin{IEEEeqnarray}{rCl}
 ...
 & \propto & \pw{\pt_0} \times \frac{ \obsden(\ob{\rt} | \ls{\pt_1})^{\pt_1} \transden(\ls{\pt_1} | \ls{\rt-1}) }{ \obsden(\ob{\rt} | \ls{\pt_0})^{\pt_0} \transden(\ls{\pt_0} | \ls{\rt-1}) } \times \sqrt{\frac{\magdet{\lgoicovapprox{\pt_1}{\pt_0}}}{\magdet{\lgoicovapprox{\pt_0}{\pt_0}}}} \label{eq:PPPF_deterministic_weight_update}       .
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 \lgoicovapprox{0}{0} & = & \transcov \nonumber \\
 \lgoimeanapprox{0}{0} & = & \transfun(\ls{\rt-1}) \nonumber       .
\end{IEEEeqnarray}
%
Now, as the pseudo-time is incremented from $\pt_0$ to $\pt_1$, the following update is required,
%
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
%
Approximating this density may be achieved using methods from nonlinear Kalman filtering, for a survey of which see \cite{Sarkka2013}. Ignoring higher moments, the mean and covariance of the updated density are given by,
%
\begin{IEEEeqnarray}{rCl}
 ...
 \lgoimeanapprox{\pt_1}{\pt_0} & = & \lgoimeanapprox{\pt_0}{\pt_0} + \obcvr{\pt_0} \left(\obvr{\pt_0}+ \frac{\obscov}{\pt_1-\pt_0}\right)^{-1} \left(\ob{\rt}-\obmn{\pt_0}\right) \nonumber \\
 \lgoicovapprox{\pt_1}{\pt_0}  & = & \lgoicovapprox{\pt_0}{\pt_0}  - \obcvr{\pt_0} \left(\obvr{\pt_0}+ \frac{\obscov}{\pt_1-\pt_0}\right)^{-1} \obcvr{\pt_0} \label{eq:gaussian_moment_matching} .
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 \obmnapprox{\pt_1} & = & \obsfun(\ls{\pt_0}) + \obsmatapprox{\ls{\pt_0}} ( \lgoimeanapprox{\pt_0}{\pt_0} - \ls{\pt_0} ) \nonumber \\
 \obvrapprox{\pt_1} & = & \obsmatapprox{\ls{\pt_0}} \lgoicovapprox{\pt_0}{\pt_0} \obsmatapprox{\ls{\pt_0}}^T + \frac{\obscov}{\pt_1-\pt_0} \nonumber \\
 \obcvrapprox{\pt_1} & = & \lgoicovapprox{\pt_0}{\pt_0} \obsmatapprox{\ls{\pt_0}}^T \nonumber \\
 \lgoimeanapprox{\pt_1}{\pt_0} & = & \lgoimeanapprox{\pt_0}{\pt_0} + \obcvrapprox{\pt_1} \left(\obvrapprox{\pt_0}+ \frac{\obscov}{\pt_1-\pt_0}\right)^{-1} \left(\ob{\rt}-\obmnapprox{\pt_1}\right) \nonumber \\
 \lgoicovapprox{\pt_1}{\pt_0}  & = & \lgoicovapprox{\pt_0}{\pt_0}  - \obcvrapprox{\pt_1} \left(\obvrapprox{\pt_0}+ \frac{\obscov}{\pt_1-\pt_0}\right)^{-1} \obcvrapprox{\pt_1} \nonumber       .
% \lgoimeanapprox{\pt_1}{\pt_0} & = & \lgoimeanapprox{\pt_0}{\pt_0} + \lgoicovapprox{\pt_0}{\pt_0} \obsmatapprox{\ls{\pt_0}}^T \left( \obsmatapprox{\ls{\pt_0}} \lgoicovapprox{\pt_0}{\pt_0} \obsmatapprox{\ls{\pt_0}}^T + \frac{\obscov}{\pt_1-\pt_0}\right)^{-1} \left( \ob{\rt} - \obsfun(\ls{\pt_0}) - \obsmatapprox{\ls{\pt_0}} ( \lgoimeanapprox{\pt_0}{\pt_0} - \ls{\pt_0} ) \right) \nonumber \\
% \lgoicovapprox{\pt_1}{\pt_0}  & = & \lgoicovapprox{\pt_0}{\pt_0}  - \lgoicovapprox{\pt_0}{\pt_0} \obsmatapprox{\ls{\pt_0}}^T \left( \obsmatapprox{\ls{\pt_0}} \lgoicovapprox{\pt_0}{\pt_0} \obsmatapprox{\ls{\pt_0}}^T + \frac{\obscov}{\pt_1-\pt_0}\right)^{-1} \obsmatapprox{\ls{\pt_0}} \lgoicovapprox{\pt_0}{\pt_0} \nonumber       .
\end{IEEEeqnarray}
%
...
\subsubsection{Optimality of the Updates}

Is linearising the observation function the best option, or would other integral approximations perform better? This question may be addressed by considering the limiting behaviour as the update step sizes become small. The Fokker-Planck equation relates the motion of a particle with the evolution of its state distribution. By applying this to the OID sequence, a constraint for optimal particle dynamics is obtained, and we can assess how far this is from being met by the dynamics resulting from our chosen approximation.

Recall from \eqref{eq:state_SDE} that the continuous-time evolution of the state between two update instants may be described by an SDE. In the (purely theoretical) limit as the pseudo-time steps become infinitesimally short, this becomes,
%
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
%
in which the dependence on the approximation point has been dropped because it is recalculated continuously. Using \eqref{eq:gaussian_moment_matching}, this becomes,
%
\begin{IEEEeqnarray}{rCl}
 d
\end{IEEEeqnarray}
%
and using linearisation specifically,
%
\begin{IEEEeqnarray}{rCl}
 d
\end{IEEEeqnarray}
...
\end{IEEEeqnarray}
%
then the resulting density obeys,
%
\begin{IEEEeqnarray}{rCl}
 d
\end{IEEEeqnarray}
%
For the OID sequence,
%
\begin{IEEEeqnarray}{rCl}
 d
\end{IEEEeqnarray}
%
Hence, equating $$ and $$, the optimal state dynamics are governed by,
%
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
%
This relationship was derived in \cite{Daum2008,Reich2011}. See appendix~\ref{app:optimal_flow_governing_eq} for details.

This formula relates the parameters of the optimal flow, $\flowdrift{\pt}$ and $\flowcov{\pt}$, to the gradient and Hessian of the density at the current state, $\pdv{\logoiden{\pt}}{\ls{\pt}}$ and $\ppdv{\logoiden{\pt}}{\ls{\pt}}$, and the expected log-likelihood, $\expect{\oiden{\pt}}\left[ \logobs(\ls{}) \right]$. This expected log-likelihood is the bane of effective particle flows. It can be calculated analytically only when using a partially linear-Gaussian model, and since the progressive proposal method treats particles independently, it cannot be well approximated, since we have information about the OID only at a single point.

Simply assuming (and hoping) that the expected log-likelihood is approximated with sufficient accuracy, we can consider a choice of particle dynamics which ensures at least that the gradient and Hessian of the approximation log-density are well matched to their true values. Suppose we have an OID approximation which at pseudo time $\pt$ and particle state $\ls{\pt}$ has derivative perfectly matched to the true log-density, i.e.
%
\begin{IEEEeqnarray}{rCl}
 d
\end{IEEEeqnarray}
...
{\meta Change this:}
\begin{IEEEeqnarray}{rCl}
 \pdv{\oiden{\pt}}{\ls{\pt}}(\ls{\pt}+\dls,\pt+\dpt) & = & \pdv{\oiden{\pt}}{\ls{\pt}}(\ls{\pt},\pt) + \mpdv{\oiden{\pt}}{\ls{\pt}}{\pt}(\ls{\pt},\pt) \dpt + \ppdv{\oiden{\pt}}{\ls{\pt}}(\ls{\pt},\pt) \dls \nonumber \\
\end{IEEEeqnarray}
%
Therefore, the derivatives at the particle state will continue to be perfectly matched provided $\pdv{}{}$. Using linearisation it may be shown that,
%
\begin{IEEEeqnarray}{rCl}
 d
\end{IEEEeqnarray}
%
This is not the case for any other approximation. So using linearisation, the only term in \eqref{eq:optimal_flow_PDE} which is not satisfied is the expected log-likelihood, whereas using other methods there will be additional errors.
...
%
\begin{IEEEeqnarray}{rCl}
 \logobs(\ls{\pt}) - \expect{\oiden{\pt}}\left[ \logobs(\ls{}) \right] & = & -\trace\left[ \frac{\partial \flowdrift{\pt}}{\partial \ls{\pt}} \right] - \flowdrift{\pt}(\ls{\pt})^T \frac{\partial \logoiden{\pt}}{\partial \ls{\pt}} \nonumber \\
