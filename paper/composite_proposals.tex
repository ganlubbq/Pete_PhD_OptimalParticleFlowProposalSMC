...
\end{IEEEeqnarray}
%
where $\lgoimeanapprox{\pt}{\lsfixed}$ and $\lgoicovapprox{\pt}{\lsfixed}$ are the mean and covariance formed at the point $\lsfixed$, which are themselves functions of $\pt$. Methods for making such an approximation are discussed in section~\ref{}.
...
We first require $\stdnorm{\pt}$ to be continuous. This seems a sensible precaution since jumps in $\stdnorm{\pt}$ will cause jumps in $\ls{\pt}$ away from the approximation point. Then in order to maintain the correct marginal distribution, $\stdnorm{\pt}$ must evolve according to a stationary Ornstein-Uhlenbeck process governed by,
%
\begin{IEEEeqnarray}{rCl}
...
Plugging \eqref{eq:standard_normal_update} into \eqref{eq:gaussian_decomposition} and using a Gaussian approximation for the OID sequence, the update formula for $\ls{\pt}$ resulting from the evolution of $\stdnorm{\pt}$ is,
%
\begin{IEEEeqnarray}{rCl}
...
When $\lgexpsf=0$, the state evolution is entirely deterministic.

Henceforth, we assume that the approximation for the next update is conducted at the current state, $\lsfixed=\ls{\pt_0}$.

State updates made in this manner are only ``approximate'' in the sense that the particles are not distributed exactly according to the OID. This discrepancy is compensated for in the weight calculation, meaning the particle filter retains its asymptotic properties.
