...
%
\begin{IEEEeqnarray}{rCl}
 d\ls{\pt} & = & \left[ \frac{\partial \lgoimeanapprox{\pt}{\lsfixed}}{\partial \pt} + \half \left( \frac{\partial \lgoicovapprox{\pt}{\lsfixed} }{\partial \pt} \lgoicovapprox{\pt}{\lsfixed}^{-1} - \lgexpsf I \right) (\ls{\pt}-\lgoimeanapprox{\pt}{\lsfixed}) \right] d\pt + \lgexpsf^{\half} \lgoicovapprox{\pt}{\lsfixed}^{\half} d\lginfbm{\pt}      .
\end{IEEEeqnarray}
%
...
Later, we consider efficient ways to add resample-move steps to the CPPF. For this purpose, it will be beneficial to have $\stdnorm{\pt}$ evolving stochastically, so that multiple particles with the same original state are able to ``de-correlate'' and explore the state space.



Note that if an alternative factorisation of $\lgoicov{\pt}$ were used in \eqref{eq:gaussian_decomposition} (for example, a Cholesky decomposition), this would be equivalent to introducing an arbitrary rotation into the state update. Since this will increase the expected distance the state travels away from the approximation point, it is undesirable. {\meta Change this, move it or reference optimality section, or something.}


\subsection{Optimality and Performance Considerations}

The performance of the CPPF is highly dependent on the size of the pseudo-time steps taken. As step sizes decrease, performance improves. However, even in the limiting case as the step sizes tend to $0$, the algorithm will not generate particles exactly from the OID.
...
{\meta Explain how as the step sizes go to $0$ and provided we are using a sensible approximation, all the terms become exact apart from the expectation, which is always approximated. Perhaps move this section before the section about approximations.}
...
{\meta Reconsider, and perhaps rewrite this.}
