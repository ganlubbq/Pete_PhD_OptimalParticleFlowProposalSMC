...
%
\begin{IEEEeqnarray}{rCl}
 w_t^{(j)} & = & \frac{ p(x_{t}^{(j)}, x_{t-1}^{(b_j)} | y_{1:t}) }{ p(x_{t-1}^{(b_j)} | y_{1:-1}) q(x_t^{(j)} | x_{t-1}^{(b_j)}, y_t) } \nonumber \\
 & \propto & \frac{\bar{w}_{t-1}^{(j)}}{\bar{v}_{t-1}^{(j)}} \times \frac{ p(y_t | x_t^{(j)}) p(x_t^{(j)} | x_{t-1}^{(b_j)}) }{ q(x_t^{(j)} | x_{t-1}^{(b_j)}, y_t) }     ,
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 w_t^{(j)} & \propto & \frac{\bar{w}_{t-1}^{(j)}}{\bar{v}_{t-1}^{(j)}} \times p(y_t | x_{t-1}^{(b_j)}) \nonumber \\
           & \propto & \frac{\bar{w}_{t-1}^{(j)}}{\bar{v}_{t-1}^{(j)}} \times \int p(y_t | x_t) p(x_t | x_{t-1}^{(b_j)}) dx_t      .
\end{IEEEeqnarray}
%
...
where
\begin{IEEEeqnarray}{rCl}
 \tilde{\pi}_{t,\lambda}(x_{1:t-1}, \xtlam) & = & \frac{ p(y_t|x_t)^{\lambda} p(x_t|x_{t-1}) p(x_{1:t-1}|y_{1:t-1}) }{ \int p(y_t|x)^{\lambda} p(x|y_{1:t-1}) dx }     ,
\end{IEEEeqnarray}
%
...
Next we examine how the particle weights should be updated as the particles move according to the flow. Suppose we have a set of weighted particles at pseudo-time $\lambda$ which are distributed according to the target distribution, and consider advancing by a small pseudo-time increment, $\delta\lambda$. The particle trajectories are advanced to $\lambda+\delta\lambda$ deterministically using the flow and the weights updated in the usual way,
%
\begin{IEEEeqnarray}{rCl}
 w_{\lambda+\delta\lambda} & = & \frac{ d\tilde{\pi}_{\lambda+\delta\lambda}(x_{1:t-1}, \xldl) d\rho(\xztldl | \xldl) }{ d\tilde{\pi}_{\lambda}(x_{1:t-1}, \xlam) d\rho(\xztl | \xlam) \mathbbm{1}_{\phi_{\lambda,\lambda+\delta\lambda}(\xlam)}(\xldl) } \nonumber \\
                          & = & \frac{ d\tilde{\pi}_{\lambda+\delta\lambda}(x_{1:t-1}, \xldl) d\rho(\xlam | \xldl) }{ d\tilde{\pi}_{\lambda}(x_{1:t-1}, \xlam) \mathbbm{1}_{\phi_{\lambda,\lambda+\delta\lambda}(\xlam)}(\xldl) } \nonumber \\
                          & = & \frac{ \tilde{\pi}_{\lambda+\delta\lambda}(x_{1:t-1}, \xldl) }{ \tilde{\pi}_{\lambda}(x_{1:t-1}, \xlam) } \left|\frac{ \partial \xldl }{ \partial \xlam }\right| \frac{ d\rho(\xlam | \xldl) }{ \mathbbm{1}_{\phi_{\lambda,\lambda+\delta\lambda}(\xlam)}(\xldl) } \nonumber      .
\end{IEEEeqnarray}
%
...
%
\begin{IEEEeqnarray}{rCl}
 w(\lambda+\delta\lambda) & = & \frac{ \tilde{\pi}_{\lambda+\delta\lambda}(x_{1:t-1}, \xldl) }{ \tilde{\pi}_{\lambda}(x_{1:t-1}, \xlam) } \left|\frac{ \partial \xldl }{ \partial \xlam }\right| \nonumber       .
\end{IEEEeqnarray}
