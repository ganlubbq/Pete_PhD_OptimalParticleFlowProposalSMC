...
\begin{IEEEeqnarray}{rCcCl}
 \xlam & = & \phi(x_{0}) & = & x_{0} + \int_{0}^{\lambda} f_{l}(x_{l}) dl     .
\end{IEEEeqnarray}
...
Having found a suitable mechanism for generating particles, we next come to the question of how to weight them correctly. In this section, we denote the continuous trajectory of the state over the pseudo-time interval $[\lambda_1, \lambda_2]$ as $x_{\lambda_1:\lambda_2}$. We assume that $\flam$ satisfies conditions which ensure that there is only one trajectory which leads to a given state, $\xlam$, and we write this trajectory as $\xtraj$. {\meta I think the required condition is simply that $\flam$ be smooth.} 
...
%
\begin{IEEEeqnarray}{rCl}
 \left.\frac{\partial f}{\partial x}\right|_{x(\lambda),\lambda} & = & A     .
\end{IEEEeqnarray}
%
Hence, we can approximate the weights using the following approximation on the same numerical grid as the ODE solution.
%
\begin{IEEEeqnarray}{rCl}
 w(\lambda+\lambda\delta) & \approx & w(\lambda) \frac{ \pi_{\lambda+\delta\lambda}(x(\lambda+\delta\lambda))  }{ \pi_{\lambda}(x(\lambda))  } \left[ 1 + \delta\lambda \mathcal{TR}(A) \right]     .
\end{IEEEeqnarray}
...
If the densities are Gaussian but the observation model is nonlinear, we can linearise it and use the Gaussian flow from the previous section.
%
\begin{IEEEeqnarray}{rCl}
...
\begin{itemize}
  \item Is the weight cancellation valid. Does the required ``density'' exist?
