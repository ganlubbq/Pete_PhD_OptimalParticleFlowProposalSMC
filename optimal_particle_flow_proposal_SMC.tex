...
\end{IEEEeqnarray}
%
where the random variable $X_t$ is the time-evolving hidden state of a system, and $Y_t$ is an incomplete, noisy observation. We assume here that the transition, observation and prior densities may be evaluated and that the prior and transition densities may be sampled. The task of filtering is to estimate the distribution of each state in turn given the observations up to the current time, referred to as the filtering distribution, which has the following density,
%
\begin{IEEEeqnarray}{rCl}
 p(x_t | y_{1:t}) & = & \frac{ \int p(x_0) \prod_{k=1}^{t} p(y_k|x_k) p(x_k|x_{k-1}) dx_{1:t-1} }{ \int p(x_0) \prod_{k=1}^{t} p(y_k|x_k) p(x_k|x_{k-1}) dx_{1:t} }     .
\end{IEEEeqnarray}

If the model densities are all Gaussian, the transition density mean is only linearly dependent on the previous state, and the observation density mean is only linearly dependent on the current state, then a recursion for the filtering density is given by the Kalman filter \cite{Grewal2002}. If the dependence is nonlinear then approximations such as the EKF or UKF may be used. However, these are only reliable for weak nonlinearity.
...
\end{IEEEeqnarray}
%
where $\delta_{x_{t}^{(i)}}(x_t)$ denotes a unit probability mass at the point $x_{t}^{(i)}$. (To be precise, the associated probability measure consists of a sum of weighted indicator functions at these points.)

Particles are generated by first setting $X_{t-1}=x_{t-1}^{(i)}$ with probability $v_{t-1}^{(i)}$, and then proposing a new state $X_t$ from an importance density, $q(x_t | X_{t-1}, y_t)$. Finally, an importance weight is assigned to the particle,
%
\begin{IEEEeqnarray}{rCl}
 w_t^{(i)} & = & \frac{ p(x_{t}^{(i)}, x_{t-1}^{(i)} | y_{1:t}) }{ p(x_{t-1}^{(i)} | y_{1:-1}) q(x_t^{(i)} | x_{t-1}^{(i)}, y_t) } \nonumber \\
 & \propto & \frac{w_{t-1}^{(i)}}{v_{t-1}^{(i)}} \times \frac{ p(y_t | x_t^{(i)}) p(x_t^{(i)} | x_{t-1}^{(i)}) }{ q(x_t^{(i)} | x_{t-1}^{(i)}, y_t) }     .
\end{IEEEeqnarray}
...
\begin{IEEEeqnarray}{rCl}
 w_t^{(i)} & \propto & p(y_t | x_{t-1}^{(i)}) \nonumber \\
           & \propto & \int p(y_t | x_t) p(x_t | x_{t-1}^{(i)}) dx_t      .
\end{IEEEeqnarray}
%
...
In this section, we introduce a framework for a new method of particle filtering designed to alleviate the problem of particle degeneracy. This is achieved by introducing a continuous sequence of distributions between the prior (predictive) and posterior (filtering) distributions at each time step, and then moving the particles either deterministically of stochastically so that they are distributed accordingly. Since this allows a smooth introduction of the observation, we refer to it as the Smooth Update Particle Filter (SUPF).

First, a variable $\lambda \in [0,1]$ is introduced. Intuitively, this is a stretch of ``pseudo-time'' between the predictive and filtering distributions, allowing the effect of the observation to be introduced gradually. Define $X_{t,\lambda}$ as the state at time $t$ and pseudo-time $\lambda$, and the following sequence of densities,
%
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
%
For the $(i)$th particle, $\pitlam(\xtlam | x_{t-1}^{(i)})$ is the transition density when $\lambda=0$ and the OID when $\lambda=1$.

Further, let $X_{t,0:\lambda}$ be the continuous state trajectory over the interval $[0,\lambda)$. The target distribution for the SUPF is then chosen to be,
%
\begin{IEEEeqnarray}{rCl}
 dp(x_{t-1} | y_{1:t-1}) d\pitlam(\xtlam|x_{t-1}) d\rho(\xtztl | \xtlam)     ,
\end{IEEEeqnarray}
%
where $d\rho$ is an artificial extension to the distribution and can be chosen as a design parameter --- ultimately we only care about the distribution of the final states. Note that the target distribution is specified in terms of measures rather than densities due the fact that $d\rho$ will be chosen so as not to admit a valid density in the case of the deterministic update.

At pseudo-time $\lambda=1$, this target distribution clearly has the filtering distribution as a marginal. Once particles have been sampled from the target, we can simply discard the pseudo-time trajectories (and, if not desired, the $t-1$ states as well) to leave samples from the filtering distribution.
...
The deterministic SUPF (DSUPF) uses a non-random time-varying flow field to move each particle around the state space. The field is chosen so that the distribution of the particle, $X_{t,\lambda}^{(i)}$ is exactly or approximately equal to $\pitlam(\xtlam | x_{t-1}^{(i)})$, meaning that the variance of the weights is minimised.
...
\subsection{Piecewise Linear Flows}
...
%
\begin{IEEEeqnarray}{rCl}
 \int_{0}^{\lambda} \mathcal{TR}\left[ \left.\frac{\partial f_{\nu}}{\partial x_{\nu}}\right|_{l,x_l} \right] dl & = & -\frac{1}{2} \int_{0}^{\lambda} \mathcal{TR}\left[ Q H^T \left(R + l H Q H^T \right)^{-1} H \right] dl \nonumber \\
 & = & -\frac{1}{2} \int_{0}^{\lambda} \mathcal{TR}\left[ H Q H^T \left(R + l H Q H^T \right)^{-1} \right] dl \nonumber \\
 & = & -\frac{1}{2} \mathcal{TR}\left[ \int_{0}^{\lambda} H Q H^T \left(R + l H Q H^T \right)^{-1} dl \right] \nonumber \\
 & = & -\frac{1}{2} \mathcal{TR}\left[ \log \left(R + \lambda H Q H^T \right) - \log(R) \right]      ,
\end{IEEEeqnarray}
%
