function [ x, J, prob_ratio, drift] = linear_flow_move( t, t0, x0, m, P, y, H, R, Dscale )
...
% Transformation
Gam = exp(-0.5*Dscale*(t-t0))*sqrtm(Sigmat)*expmThedt/sqrtm(Sigma0);
Ome = (1-exp(-Dscale*(t-t0)))*Sigmat;
x_mn = mut + Gam*(x0-mu0);
...
if Dscale ~= 0
    x = mvnrnd(x_mn', Ome)';
    J = 0;
    prob_ratio = loggausspdf(x0,mu0,Sigma0)-loggausspdf(x,mut,Sigmat);
else
    x = x_mn;
    J = sqrt(det(Sigmat)/det(Sigma0));
    prob_ratio = 0;
end
